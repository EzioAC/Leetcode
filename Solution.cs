...
namespace Leetcode
{
    ...
    public class Solutions
    {
            ...
            for(int i = 0;i<points.Length;i++)
            {
                ...
                for(int j = 0;j<points.Length;j++)
                {
                    ...
                    { 
                        ...
                        x /= g;
                        y /= g;
                    if(!count.ContainsKey(x.ToString() + " " + y.ToString()))
                        count[x.ToString() + " " + y.ToString()] = 0;
                    count[x.ToString() + " " + y.ToString()]++;
                }
                max = Math.Max(max,same);
                foreach(var key in count.Keys)
                {
                    max = Math.Max(max,count[key]+same);
                }
                count.Clear();
            }
            return max;
        ...
        public int[] MaxSlidingWindow(int[] nums, int k)
        {
            ...
            for(int i = 0;i<k-1;i++)
            {
                while(list.Last!=null&&list.Last.Value[0]<=nums[i])
                    list.RemoveLast();
                list.AddLast(new int[2]{nums[i],i});
            }
            for(int i = 0;i<r.Length;i++)
            {
                if(list.First!=null&&list.First.Value[1]<i)
                    list.RemoveFirst();
                while(list.Last!=null&&list.Last.Value[0]<=nums[i+k-1])
                    list.RemoveLast();
                list.AddLast(new int[2]{nums[i+k-1],i+k-1});
                r[i] = list.First.Value[0];
            }
            return r;
        }
        ...
        public IList<int[]> GetSkyline(int[,] buildings)
        {
            Stack<int[]> stack = new Stack<int[]>();
            int i = 0;
            for(i = 0;i<buildings.GetLength(0);i++)
            {
                if(stack.Count<0||stack.Peek()[0]<buildings[i,0])
                {
                    stack.Push(new int[]{buildings[i,0],buildings[i,2]});
        ...
        private void Add(ref int index,Stack<int[]> stack,int[,] buildings) 
        {
            stack.Push(new int[]{buildings[index,0],buildings[index,2]});
            
            stack.Push(new int[]{buildings[index,1],buildings[index,2]});
        }
